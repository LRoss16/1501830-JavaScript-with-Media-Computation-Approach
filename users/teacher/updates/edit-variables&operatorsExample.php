...
<head>
  ...
  <title>Admin - Edit Variables & Operators Code Example</title>
...
<div id="wrapper">
	...
	if(isset($_POST['submit'])){
		...
		if(!isset($error)){
			...
			try {
				...
				$stmt = $db->prepare('UPDATE variablesOperators SET postDate = :postDate, codeExample=:codeExample WHERE postID = :postID') ;
	...
	if(isset($error)){
		...
		try {
			...
			$stmt = $db->prepare('SELECT postID, codeExample FROM variablesOperators WHERE postID = :postID') ;
