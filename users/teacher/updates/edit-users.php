...
//check teacher field is correct
		if ($row['memberType'] == 2) {	
  ...
  $mail->Username = '*********';
   ...
   $mail->Port = 587;
       ...
       if ($mail->addReplyTo($_POST['email'])) {
        ...
        $mail->isHTML(true);
        // email admin about account change
  ...
  $mail->Username = '************';
   ...
   $mail->Port = 587;
       ...
       if ($mail->addReplyTo($_POST['email'])) {
        $mail->Subject = 'JSMC Account';
        $mail->isHTML(true);
        // email admin about account changes
        $mail->Body = "Account details have changed for a user: 
         <br> Their username is: $username"; 		 
...
//get user details
	...
	<form action='' method='post'>
		...
		<input type='text' readonly name='teacher' value='<?php echo $row['teacher'];?>'></p>
