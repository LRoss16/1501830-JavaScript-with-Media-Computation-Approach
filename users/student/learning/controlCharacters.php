...
<div class="main">
 ...
 //get content set up by students teacher
			...
			$state = $db->prepare('SELECT postTitle, postCont FROM controlCharacters WHERE editedBy= :teacher') ;
...
<script>
 ...
 //get code example set up by students teacher
			...
			$state = $db->prepare('SELECT codeExample FROM controlCharacters WHERE editedBy= :teacher') ;
			$state->bindParam(':teacher', $teacher, PDO::PARAM_STR);
                         $state->execute();
			while($content= $state->fetch()) {
