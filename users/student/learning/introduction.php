...
<div class="main">
		...
		//get content set up by students teacher
			...
			$state = $db->prepare('SELECT postTitle, postCont FROM introduction WHERE editedBy= :teacher') ;
...
<script>
     ...
     try {
		 ...
		 //get canvas exmaple set up by students teacher
			...
			$state = $db->prepare('SELECT canvasExample FROM introduction WHERE editedBy= :teacher') ;
			$state->bindParam(':teacher', $teacher, PDO::PARAM_STR);
                         $state->execute();
			while($content= $state->fetch()) {


               echo  $content['canvasExample'];
	}		   
		} catch(PDOException $e) {
