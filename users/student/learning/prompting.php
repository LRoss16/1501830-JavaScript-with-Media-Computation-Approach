...
<div class="main">
 <?php
			...
			$state = $db->prepare('SELECT postTitle, postCont FROM prompting WHERE editedBy= :teacher') ;
...
<script>
 <?php
			...
			$state = $db->prepare('SELECT codeExample FROM prompting WHERE editedBy= :teacher') ;
			$state->bindParam(':teacher', $teacher, PDO::PARAM_STR);
                         $state->execute();
			$content= $state->fetch(); 
