<!--This page will display the content for learning of strings -->
<!DOCTYPE html>
<?php
//connect to database
...
<div class="main">
 ...
 //connect to database
			try {
				...
				$stmt = $db->query('SELECT postTitle, postCont  FROM strings WHERE editedBy = "admin" ');
...
<script>
 ...
 //get code example from database
			try {
				...
				$stmt = $db->query('SELECT codeExample  FROM strings WHERE editedBy = "admin" ');
				while($row = $stmt->fetch()){
